...
set "ProjectName=Mtf.LanguageService.WPF"
set "TargetDir=C:\NuGetTest"

for %%F in ("%TargetDir%\Mtf.WPF.LanguageService.*.nupkg") do (
    set "FileName=%%~nxF"
...
for %%F in ("%TargetDir%\Mtf.WPF.LanguageService.*.nupkg") do call :ProcessFile "%%F"
goto :UpdatePackages
...
call set "RestPart=%%FileName:Mtf.WPF.LanguageService.=%%"
